...
describe("parseLinkRequest", () => {
  it("accepts a well-formed read invite", () => {
    expect(parseLinkRequest({ appId: "app_1", requested: "read" })).toEqual({
      appId: "app_1",
      requested: "read",
    });
  });

  it("accepts a well-formed read-write invite", () => {
    expect(parseLinkRequest({ appId: "app_1", requested: "read-write" })).toEqual({
      appId: "app_1",
  ...
  it("strips extra junk, returning only the typed shape", () => {
    expect(
      parseLinkRequest({ appId: "app_1", requested: "read", extra: 9, nested: { x: 1 } }),
  ...
  it.each([
    ...
    expect(parseLinkRequest(input)).toBeNull();
  });
});
