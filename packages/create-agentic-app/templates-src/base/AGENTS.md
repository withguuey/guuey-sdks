...
(`src/worker.ts`'s agent pod, and every `mcps/*` server deployed with
`kind: hosted`) is **ephemeral and horizontally replaced**: gVisor-isolated,
...
**federated** — that's the opt-in. A `kind: hosted` entry (like
`mcps/todo` in this scaffold) is federated automatically. If you add a
`kind: external` entry (a server you host yourself, reached by URL), you
must set `federate: true` on it:
...
Without `federate: true` (or `kind: hosted`/a `ggui` URL), guuey has no
